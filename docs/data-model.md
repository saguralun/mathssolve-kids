...
### เติมชุดตัวเลขอัตโนมัติจาก JSON

นอกจากกรอกทีละช่องในตาราง Variables แล้ว หน้า import มีช่อง **"วาง variant เป็น JSON เพื่อเติมอัตโนมัติ"** ไว้เติมหลายชุดพร้อมกันในคลิกเดียว

วางข้อความแบบนี้ (มี prefix `variant N:` หรือไม่มีก็ได้ ระบบสนใจแค่ก้อน `{...}`):

```text
variant 1: {"math1":"3:5","math2":"8:11","math3":"A + B + C","answer":"128"}
variant 2: {"math1":"4:7","math2":"3:5","math3":"A - B + C","answer":"6"}
variant 3: {"math1":"5:6","math2":"7:9","math3":"A + B - C","answer":"19"}
```

แล้วกด "เติมอัตโนมัติจาก JSON" ระบบจะ:

- อ่านจำนวนชุดจากจำนวนบรรทัด (สูงสุด 5 ชุด ตามข้อจำกัดของ `problem_variants`)
- สร้างช่องตัวแปรให้ตรงกับคีย์ที่เจอในแต่ละก้อน (เรียงตามเลขท้ายชื่อ เช่น `math1`, `math2`, `math3`) โดยไม่ต้องกด "+ เพิ่ม part" เอง
- เติมค่าและ `answer` ให้ครบทุกชุดอัตโนมัติ
- **ไม่แตะ** ข้อความ part (ประโยคโจทย์คงที่), เรื่อง, หรือระดับความยาก — ต้องกรอก/เลือกเองแยกต่างหากเสมอ

ค่าตัวแปรไม่จำเป็นต้องเป็นตัวเลขล้วน ใส่เป็น string อะไรก็ได้ (เช่น `"3:5"` ทั้งก้อน หรือข้อความคำถามอย่าง `"A + B - C"`) เพราะ `variant_values` เก็บทุกอย่างเป็น string อยู่แล้ว และค่าจะถูกแทนที่ `{mathN}` ในประโยคตรงๆ

### วางโจทย์แม่เต็มข้อความเพื่อแบ่ง part อัตโนมัติ

ฝั่ง "ตัวต่อโจทย์" (Pattern) ก็มีช่องวางข้อความเต็มให้เหมือนกัน แทนที่จะพิมพ์ทีละ part เอง วางข้อความที่มี `{math1}`, `{math2}`, ... ไว้แล้วได้เลย เช่น:

```text
อาคารอเนกประสงค์แห่งหนึ่งมีอัตราส่วนของความกว้างต่อความยาวเป็น {math1} และความสูงต่อความยาวเป็น {math2} ถ้าอัตราส่วนของความกว้างต่อความยาวต่อความสูง เขียนเป็น A : B : C (อย่างต่ำ) แล้ว {math3} มีค่าเท่ากับเท่าไร
```

กด "แบ่ง part อัตโนมัติ" แล้วระบบจะตัดข้อความตรง `{mathN}` ให้เป็น part ให้เอง ข้อความที่เหลือหลังตัวแปรตัวสุดท้าย (เช่น "มีค่าเท่ากับเท่าไร") จะถูกเก็บเป็น part เพิ่มแบบไม่มีตัวแปร เพื่อไม่ให้หายไปตอนวาง JSON ทับทีหลัง

**ลำดับที่แนะนำ:** วางโจทย์แม่เต็มข้อความก่อน แล้วค่อยวาง/กรอกชุดตัวเลขทีหลัง (สลับลำดับได้ แต่การวางโจทย์แม่ใหม่จะล้างชุดตัวเลขที่กรอกไว้ก่อนหน้าเสมอ)
