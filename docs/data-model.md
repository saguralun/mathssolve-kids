...
| `source_name` | แหล่งที่มาโจทย์ เช่น `TEDET`, `สสวท`, ชื่อโรงเรียน — เป็นข้อความอิสระ ไม่บังคับกรอก ไม่มี master table แยก (ลองทำมาก่อนแล้วพบว่าซับซ้อนเกินจำเป็น) หน้า import จะดึงชื่อที่เคยพิมพ์ไว้แล้วมาเป็น autocomplete ให้ |
| `answer_decimal_places` | 0 = คำตอบเป็นจำนวนเต็ม (ค่าเริ่มต้น) 1-4 = เลื่อนจุดทศนิยมเข้ามาจากขวากี่ตำแหน่งตอนแสดงช่องตอบ 5 หลักในหน้าเล่นโจทย์ |
| `hint_id` | อ้างไปหา `problem_hints` (ไม่บังคับ) — เช่น "กำหนดให้ π ≈ 22/7" จะถูกเติมท้ายโจทย์ให้อัตโนมัติตอนแสดงผล ไม่ต้องพิมพ์ลงใน `prompt_template_th` |
| `diagram_type` | รูปประกอบ (ไม่บังคับ) — ค่าที่รองรับตอนนี้: `box` วาดเป็น SVG จากตัวแปร `diagramA`/`diagramB`/`diagramC` (กว้าง/ยาว/สูง) ใน `variant_values` ของแต่ละชุด ไม่ใส่ตัวไหนแสดงเป็น 0 ไม่ใช่ไฟล์รูปอัปโหลด เพราะแต่ละ variant มีตัวเลขไม่เหมือนกัน |
| `is_active` | เปิด/ปิดการใช้งาน |

### `problem_hints`

master table ของ hint ที่กำกับท้ายโจทย์อัตโนมัติ (ตอนนี้มีแค่ค่าประมาณ pi 2 แบบ) เพิ่มแถวใหม่ได้เรื่อยๆ ในอนาคตโดยไม่ต้องแก้ schema

| column | ใช้ทำอะไร |
| --- | --- |
| `code` | รหัส hint เช่น `pi_22_7` |
| `hint_text` | ข้อความที่จะเติมท้ายโจทย์ เช่น `กำหนดให้ π ≈ 22/7` (ระบบจะครอบวงเล็บให้เองตอนแสดงผล) |
| `sort_order` | ลำดับการแสดงผลในดรอปดาวน์หน้า import |
| `is_active` | เปิด/ปิดการใช้งาน |
